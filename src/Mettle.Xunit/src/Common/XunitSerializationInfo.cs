...
{
    /// <summary>
    /// A mirror class of the CLR's <see cref="System.Runtime.Serialization.SerializationInfo"/> class.
    /// </summary>
    internal class XunitSerializationInfo : IXunitSerializationInfo
    {
        ...
        private readonly IDictionary<string, XunitSerializationTriple> data =
            new Dictionary<string, XunitSerializationTriple>();

        /// <summary>
        /// Initializes a new instance of the <see cref="XunitSerializationInfo"/> class.
        /// </summary>
        /// <param name="instance">The data to copy into the serialization info.</param>
        public XunitSerializationInfo(IXunitSerializable instance = null)
        {
            if (instance != null)
...
#pragma warning restore CA1305
        ...
        public void AddValue(string key, object value, Type type = null)
        {
            ...
            this.data[key] = new XunitSerializationTriple(key, value, type);
        }
        ...
        public string ToSerializedString()
        {
            return ToBase64(
                string.Join("\n",
                    this.data.Select(kvp => SerializeTriple(kvp.Value)).ToArray()));
        }
        ...
        private static IXunitSerializable DeserializeSerializable(
            Type type,
        ...
        internal class ArraySerializer : IXunitSerializable
        {
            ...
            public Array ArrayData => this.array;

            public void Serialize(IXunitSerializationInfo info)
            {
                info.AddValue("ElementType",
                    SerializationHelper.GetTypeNameForSerialization(this.elementType));
                info.AddValue("Rank", this.array.Rank);
                info.AddValue("TotalLength", this.array.Length);

                for (int dimension = 0; dimension < this.array.Rank; dimension++)
                {
                    info.AddValue("Length" + dimension, this.array.GetLength(dimension));
                }

                for (int dimension = 0; dimension < this.array.Rank; dimension++)
                {
                    info.AddValue("LowerBound" + dimension, this.array.GetLowerBound(dimension));
                }
            ...
            public void Deserialize(IXunitSerializationInfo info)
            {
                ...
                this.array = Array.CreateInstance(arrType, lengths, lowerBounds);
                ...
                for (int i = 0; i < totalLength; i++)
                {
                    ...
                    this.array.SetValue(item, indices);
                    indices[rank - 1]++;
                }
            }
        }
